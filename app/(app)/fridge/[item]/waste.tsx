...
type WasteReason = 'expired' | 'spoiled' | 'accidentally_discarded' | 'other';

type ReasonOption = Readonly<{
  apiValue: WasteReason;
  label: string;
}>;

const REASON_OPTIONS: readonly ReasonOption[] = [
  { apiValue: 'expired', label: 'Expired' },
  { apiValue: 'spoiled', label: 'Spoiled' },
  { apiValue: 'accidentally_discarded', label: 'Discarded by accident' },
...
export default function WasteScreen(): JSX.Element {
  ...
  const [selectedReason, setSelectedReason] = useState<WasteReason | null>(null);
  ...
  const handleWaste = async (): Promise<void> => {
    ...
    try {
      const response = await fetch(
        `${appConfig.supabaseUrl}/functions/v1/fridge-items/households/${householdId}/fridge/items/${itemId}/waste`,
        {
      ...
      if (!response.ok) {
        ...
        const message = payload?.error?.message ?? 'Could not log waste. Please try again.';
      ...
      >
        ...
        <View style={{ gap: theme.spacing.xl, marginTop: theme.spacing.xl }}>
          ...
          <View style={{ gap: theme.spacing.sm }}>
            ...
            >
              <TextInput
                accessibilityLabel="Quantity to waste"
                allowFontScaling
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: theme.spacing.sm }}>
              {REASON_OPTIONS.map((option) => (
                <Chip
                  ...
                  label={option.label}
                  onPress={() => {
                    setSelectedReason(selectedReason === option.apiValue ? null : option.apiValue);
                  }}
                  variant={selectedReason === option.apiValue ? 'active' : 'neutral'}
                />
              ))}
            </View>
          </View>
