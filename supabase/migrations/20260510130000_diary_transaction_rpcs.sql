-- Wave 3 diary transactional write paths.
-- The RPCs are callable only by the backend service role. Edge Functions still
-- authenticate users and enforce household membership before invoking them,
-- while the database keeps diary entry, summary, event, and idempotency writes
-- in one transaction.
...
BEGIN
  SELECT
    ...
    COALESCE(SUM(carbs_mg)::integer, 0) AS total_carbs_mg,
    COALESCE(SUM(fat_mg)::integer, 0) AS total_fat_mg,
    COALESCE(SUM(kcal)::integer, 0) AS total_kcal,
    COALESCE(SUM(protein_mg)::integer, 0) AS total_protein_mg
  INTO v_totals
  FROM public.diary_entries
  WHERE user_id = p_user_id
    AND logged_at >= p_date::timestamptz
    AND logged_at < (p_date + 1)::timestamptz;
...
CREATE OR REPLACE FUNCTION private.map_diary_entry_response(
...
AS $$
  SELECT jsonb_build_object(
    'base_unit', p_entry.base_unit,
    'carbs_mg', p_entry.carbs_mg,
    'confidence', p_entry.confidence,
    'corrects_entry_id', p_entry.corrects_entry_id,
    'created_at', p_entry.created_at,
    'fat_mg', p_entry.fat_mg,
    'food_name', p_entry.food_name,
    'household_id', p_entry.household_id,
    'id', p_entry.id,
    'is_correction', p_entry.is_correction,
    'is_quick_estimate', p_entry.is_quick_estimate,
    'kcal', p_entry.kcal,
    'logged_at', p_entry.logged_at,
    'protein_mg', p_entry.protein_mg,
    'quantity_base', p_entry.quantity_base,
    'source_scope', p_entry.source_scope
  );
$$;

CREATE OR REPLACE FUNCTION private.map_daily_summary_response(
  p_summary public.daily_summaries
)
RETURNS jsonb
LANGUAGE sql
STABLE
...
AS $$
  SELECT jsonb_build_object(
    'entry_count', p_summary.entry_count,
    'total_carbs_mg', p_summary.total_carbs_mg,
    'total_fat_mg', p_summary.total_fat_mg,
    'total_kcal', p_summary.total_kcal,
    'total_protein_mg', p_summary.total_protein_mg
  );
$$;

CREATE OR REPLACE FUNCTION private.envelope_response(
  p_data jsonb,
  p_error jsonb,
  p_operation_id uuid
)
...
REVOKE ALL ON FUNCTION public.create_diary_entry_transaction(uuid, uuid, jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.correct_diary_entry_transaction(uuid, uuid, uuid, jsonb) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.create_diary_entry_transaction(uuid, uuid, jsonb) TO service_role;
GRANT EXECUTE ON FUNCTION public.correct_diary_entry_transaction(uuid, uuid, uuid, jsonb) TO service_role;
